...
export function getSelectRow(props) {
  const { entities, ids, setIds, idFieldName } = props;
  return {
    mode: "checkbox",
    clickToSelect: true,
    hideSelectAll: false,
    ...
    selectionRenderer: ({ rowIndex }) => {
      ...
      let props = {}
      if (entities != null && entities.length > 0 && entities[rowIndex]) {
        isSelected = ids.some((el) => el === entities[rowIndex][idFieldName]);
        props = { ids, setIds, customerId: entities[rowIndex][idFieldName] };
      }
      return (
        <SelectionCheckbox
          isSelected={isSelected}
