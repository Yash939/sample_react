...
export function SubHeader() {
  ...
  const layoutProps = useMemo(() => {
    return {
      ...
      displayDaterangepicker: objectPath.get(
        uiService.config,
        "subheader.displayDaterangepicker"
      ),
      displayQuickActions: objectPath.get(
        uiService.config,
  ...
  const { listLoading } = useSelector(state => {
    return {
      listLoading: state.moduleMaster.listLoading
    }
  })

  useLayoutEffect(() => {
    const aside = getBreadcrumbsAndTitle("kt_aside_menu", location.pathname, location);
    const header = getBreadcrumbsAndTitle("kt_header_menu", location.pathname, location);
    ...
    >
      ...
      >
        ...
        <div className="d-flex align-items-center">
          {layoutProps.displayDaterangepicker && (
          ...
          <QuickUserToggler/>
          {layoutProps.displayQuickActions && (
            <QuickActions />
          )}
        </div>
        </div>
