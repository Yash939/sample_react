...
const StateMasterBulkEditPage = () => {
    const dispatch = useDispatch()
    const history = useHistory()

    const { stateMasterState } = useSelector((state) =>
    ({
    ...
    const entities = useMemo(() => {
        if (stateMasterState?.entities) {
            if (bulkOption === "ACTIVE") {
                const tmpData = stateMasterState?.entities?.filter(x => x.active === true).map((x, index) => ({ ...x, keyField: index, countryName: x?.countryMST?.countryName }))
                return tmpData
            } else {
                const tmpData = stateMasterState?.entities?.filter(x => x.active === false).map((x, index) => ({ ...x, keyField: index, countryName: x?.countryMST?.countryName }))
                return tmpData
            }

        }
        return []
    }, [stateMasterState, bulkOption])
    ...
    return (
        <Formik
            ...
            onSubmit={(values) => {
                ...
                stateMasterActions.updateMultiple(val).then(res => {
                    history.goBack()
                }).catch(err => {
...
export default StateMasterBulkEditPage;