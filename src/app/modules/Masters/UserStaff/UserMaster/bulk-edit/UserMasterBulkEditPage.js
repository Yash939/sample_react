...
const UserMasterBulkEditPage = () => {
    const dispatch = useDispatch()
    const history = useHistory()

    const { userMasterState } = useSelector((state) =>
    ({
    ...
    const entities = useMemo(() => {
        if (userMasterState?.entities) {
            if (bulkOption === "ACTIVE") {
                const tmpData = userMasterState?.entities?.filter(x => x.active === true).map((x, index) => ({ ...x, 
                    keyField: index,
                    organizationName: x?.organizationMST?.organizationName,
                    roleName: x?.userRoleMST?.roleName,
                    cityName: x?.cityMST?.cityName,
                    stateName: x?.stateMST?.stateName,
                    countryName: x?.countryMST?.countryName

                }))
                return tmpData
            } else {
                const tmpData = userMasterState?.entities?.filter(x => x.active === false).map((x, index) => ({ ...x, 
                    keyField: index,
                    organizationName: x?.organizationMST?.organizationName,
                    roleName: x?.userRoleMST?.roleName,
                    cityName: x?.cityMST?.cityName,
    ...
    return (
        <Formik
            ...
            onSubmit={(values) => {
                ...
                userMasterActions.updateMultiple(val).then(res => {
                    history.goBack()
                }).catch(err => {
...
export default UserMasterBulkEditPage;