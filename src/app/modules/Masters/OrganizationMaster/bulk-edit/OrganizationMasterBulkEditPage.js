...
const OrganizationMasterBulkEditPage = () => {
    const dispatch = useDispatch()
    const history = useHistory()

    const { organizationMasterState } = useSelector((state) =>
    ({
    ...
    const entities = useMemo(() => {
        if (organizationMasterState?.entities) {
            if (bulkOption === "ACTIVE") {
                const tmpData = organizationMasterState?.entities?.filter(x => x.active === true).map((x, index) => ({ ...x, keyField: index }))
                return tmpData
            } else {
                const tmpData = organizationMasterState?.entities?.filter(x => x.active === false).map((x, index) => ({ ...x, keyField: index }))
                return tmpData
            }

        }
        return []
    }, [organizationMasterState, bulkOption])
    ...
    return (
        <Formik
            ...
            onSubmit={(values) => {
                ...
                organizationMasterActions.updateMultiple(val).then(res => {
                    history.goBack()
                }).catch(err => {
...
export default OrganizationMasterBulkEditPage;