...
const CityMasterBulkEditPage = () => {
    const dispatch = useDispatch()
    const history = useHistory()

    const { cityMasterState } = useSelector((state) =>
    ({
    ...
    const entities = useMemo(() => {
        if (cityMasterState?.entities) {
            if (bulkOption === "ACTIVE") {
                const tmpData = cityMasterState?.entities?.filter(x => x.active === true).map((x, index) => ({ ...x, keyField: index }))
                return tmpData
            } else {
                const tmpData = cityMasterState?.entities?.filter(x => x.active === false).map((x, index) => ({ ...x, keyField: index }))
                return tmpData
            }

        }
        return []
    }, [cityMasterState, bulkOption])
    ...
    return (
        <Formik
            ...
            onSubmit={(values) => {
                ...
                cityMasterActions.updateMultiple(val).then(res => {
                    history.goBack()
                }).catch(err => {
...
export default CityMasterBulkEditPage;