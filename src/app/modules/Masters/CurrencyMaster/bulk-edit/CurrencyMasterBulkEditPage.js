...
const CurrencyMasterBulkEditPage = () => {
    const dispatch = useDispatch()
    const history = useHistory()

    const { currencyMasterState } = useSelector((state) =>
    ({
    ...
    const entities = useMemo(() => {
        if (currencyMasterState?.entities) {
            if (bulkOption === "ACTIVE") {
                const tmpData = currencyMasterState?.entities?.filter(x => x.active === true).map((x, index) => ({ ...x, keyField: index }))
                return tmpData
            } else {
                const tmpData = currencyMasterState?.entities?.filter(x => x.active === false).map((x, index) => ({ ...x, keyField: index }))
                return tmpData
            }

        }
        return []
    }, [currencyMasterState, bulkOption])
    ...
    return (
        <Formik
            ...
            onSubmit={(values) => {
                ...
                currencyMasterActions.updateMultiple(val).then(res => {
                    history.goBack()
                }).catch(err => {
...
export default CurrencyMasterBulkEditPage;