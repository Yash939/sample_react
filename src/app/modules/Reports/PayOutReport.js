...
const PayOutReport = () => {
  ...
  const cols = [
    ...
    //     text: "Settlement",
    //     headerStyle: screenSize ? headerButtonStyles : { whiteSpace: 'nowrap', position: 'sticky', right: '0', backgroundColor: 'white', minWidth: '90px', maxWidth: '90px' },
    //     style: screenSize ? {} : { position: 'sticky', right: '0', backgroundColor: 'white', minWidth: '90px', maxWidth: '90px', textAlign: 'center' },
    //     formatter: (cell, row) => {
    //         return (
    //             <input type="checkbox" value={cell}
    //                 onChange={(e) => {
  ...
  useEffect(() => {
    document.title = "Due Pay Out Report";
  ...
  const rows = useMemo(() => {
    if (!Array.isArray(tmpRows)) return [];
    try {
      let qObj = {};
      if (searchText)
        cols.forEach((col) => {
      ...
      return searchInArray(tmpRows, qObj, []);
    } catch (error) {
      console.log(error);
      return [];
    }
  ...
  useEffect(() => {
    setFilterDataError("");
    dispatch(reportsActions.getPayOutFilterData())
      .then((res) => {
        setFilterData(res);
      })
  ...
  useEffect(() => {
    if (filter === "pending-payout-tickets") {
      document.title = "Pending Pay Out Tickets";
  ...
  return (
    <Formik
      ...
      validationSchema={
        filter === "pending-payout-tickets" ? null : validationSchema
      }
      onSubmit={(values) => {
        setParentValues(values);
        if (filter === "pending-payout-tickets") {
          dispatch(
            reportsActions.getData("payOutReport/noDates", values, "get")
          );
        } else {
          dispatch(reportsActions.getData("payOutReport", values, "post"));
        }
      }}
    >
      {({ handleSubmit, handleReset, setFieldValue, values }) => (
        <Form className="form form-label-right">
          <Card>
            <CardHeader title="Due Pay Out Report">
              <CardHeaderToolbar>
                {
                  <>
                    <button
            ...
            <CardBody id="summary-report">
              ...
              >
                <div className="form-group row">
                  {filter === "pending-payout-tickets" ? null : (
                    <>
                ...
                <Formik
                  ...
                  onSubmit={(values) => {
                    ...
                    //     payOutPaidCurrencyId: x.payOutPaidCurrencyId,
                      ...
                      <div className="row mb-2">
                        ...
                        <div className="col-lg-2 col-md-3 settlement">
                          <button
                            ...
                            // style={authState?.user?.organizationMST?.organizationType === "SELF" || authState?.roleCode === "admin" ? {} : { display: 'none' }}
                          >
                            <i
                              className="fa fa-database"
                              style={{ color: "#777" }}
